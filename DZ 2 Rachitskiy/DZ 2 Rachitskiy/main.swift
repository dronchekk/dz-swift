...
func numChetNechet(number:Int) -> String {
    ...
    var numberNechet = ""
    if number % 2 == 0 {
        numberChet = "Число четное!"
       return numberChet
    }
    else {
        numberNechet = "Число нечетное"
        return numberNechet
    }
}
...
func delenieBezOstatka(number:Int) -> String {
    ...
    if number % 3 == 0 {
        numberdelenieBezOstatka = "Число делится на 3 без остатка!"
       return numberdelenieBezOstatka
    }
    else {
        numberdelenieSostatkom = "Число не делится на 3 без остатка"
